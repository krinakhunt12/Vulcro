...
export default function AdminLoginPage() {
  ...
  const onSubmit = async (values) => {
    try {
      // log minimal info and send a clean payload to avoid accidental circular refs
      console.log('Admin login submit', { email: values.email });
      setLoading(true);
      ...
      try {
        ...
        console.warn('Could not parse JSON from /api/admin/login response', e);
      }

      console.log('Login response', res.status, json?.message ?? json);
      if (!res.ok) {
        toast?.push({ title: 'Login failed', description: json.message || 'Invalid credentials', variant: 'destructive' });
        return;
      }
      toast?.push({ title: 'Logged in', description: 'Welcome back', variant: 'success' });
      // we don't receive admin info from API; set minimal state and rely on cookie
      setAdmin({ email: values.email });
      // Force a full-page navigation so the browser sends/receives cookies
      // and middleware can validate the new session on the server side.
          ...
          <div className="bg-white p-8 rounded-lg shadow-md border border-gray-100">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="text-center text-sm text-gray-500">
                <a href="/admin/signup" className="text-black underline">Create an admin account</a>
              </div>
            </form>
          </div>
        </div>
